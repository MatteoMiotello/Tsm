        ...
        <div class="flex justify-between my-auto w-full" v-if="currentEvent">
            ...
            <TransitionGroup name="list" class="flex flex-row items-baseline" tag="ul">
                    <li v-for="event in currentEvents.slice( 0, 3 )" :key="event">
                        <img v-bind:src="event.thumbnail.url" v-bind:alt="event.thumbnail.alt"
                             class="aspect-[1/1.42] ml-2 backdrop-opacity-95 backdrop-invert bg-white/30 transition ease-in-out delay-500 h-64"
                             :class="currentEvent.id != event.id ? 'filter grayscale saturate-50 opacity-75 transition-all' : 'transition-all'">
                    </li>
            </TransitionGroup>
